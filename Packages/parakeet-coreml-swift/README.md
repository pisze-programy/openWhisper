...
> **On-device speech-to-text for iOS & macOS, built on NVIDIA Parakeet TDT
> 0.6B v3 (Core ML).** Maintained by
> [pisze-programy](https://github.com/pisze-programy) (Chris does apps) as a
> fork of
> [`mweinbach/parakeet-coreml-swift`](https://github.com/mweinbach/parakeet-coreml-swift),
> tuned for iPhones: faster vDSP mel extraction, GPU-first model loading,
> robust short-clip handling and decoder confidence scoring.
> Full list of changes: [Changes.md](Changes.md).

Swift wrapper around NVIDIA's **Parakeet TDT 0.6B v3** compiled to Core ML,
tuned so it's **3× faster than running the same `.mlpackage` from Python**
on the GPU.

- **iOS 17+ / macOS 14+**
- **1145× real-time** on GPU / **402× on ANE** / **163× on CPU** (M5 Max,
  17.5-min audio, 3-run median — upstream benchmark)
- **~480 MB on-disk**, compiles to `.mlmodelc`; sources can be deleted
  (`deleteSourceAfterCompile`) to save ~480 MB
- **Decoder confidence** (`Transcription.confidence`) for short-clip gating
...
let transcriber = try await ParakeetTranscriber.fromHuggingFace(
    computeUnits: .gpu           // or .ane, .cpu, .all
)
...
First call downloads ~480 MB from HuggingFace
(`mweinbach1/parakeet-tdt-0.6b-v3-coreml`) to the app's cache and compiles each
`.mlpackage` into a `.mlmodelc` cache. Subsequent runs hit the cache
instantly (~0.2 s cold start on macOS; a few seconds on iPhone for the GPU
shader warm-up).
...
2. Paste the repo URL:
   `https://github.com/pisze-programy/parakeet-coreml-ios`
...
```swift
dependencies: [
    .package(url: "https://github.com/pisze-programy/parakeet-coreml-ios",
             from: "0.1.0"),
]
...
.target(
    name: "YourApp",
    dependencies: [
        .product(name: "ParakeetTDT", package: "parakeet-coreml-ios"),
    ]
)
```
...
`ParakeetTranscriber.fromHuggingFace(...)` downloads a ~480 MB model
on first run, so if your app runs inside the App Sandbox:
...
| `confidence` | `Float` | Mean decoder confidence (0…1); gate short clips with it. |
...
|---|---|---|
| `.ane` | `cpuAndNeuralEngine` | Best power/latency balance on paper; on iOS prefer `.gpu` (ANE compilation can fail on-device — see Changes.md). |
| `.gpu` | `cpuAndGPU` | Fastest on M-class Macs (~3× ANE) and the reliable default on iPhones. |
...
## Benchmarks (upstream, M5 Max, 17.5-min MLK "I Have a Dream", 3-run median)
...
See [OPTIMIZATIONS.md](OPTIMIZATIONS.md) for the original author's journey
(from a naive port at 115× GPU to 1145× GPU) and [Changes.md](Changes.md)
for this fork's on-device improvements.
...
│   ├── GreedyTDTDecoder.swift      Greedy TDT state machine + confidence
│   ├── AudioLoader.swift           AVFoundation -> 16 kHz mono [Float]
│   ├── MelFeatureExtractor.swift   vDSP log-mel (buffer-reuse + vDSP_mmul)
...
`deleteSourceAfterCompile` is safe across launches in this fork: the compiled
bundle is found in the cache even when the source is gone (see Changes.md §4).
...
- [`mweinbach/parakeet-coreml-swift`](https://github.com/mweinbach/parakeet-coreml-swift)
  — the upstream package this repo is forked from (credit to the original author).
- [`mweinbach/parakeet-coreml`](https://github.com/mweinbach/parakeet-coreml)
  ...
  artifacts (quant / palettization experiments, MIL dumps, RTFx benchmarks).
